...
数据库结构以 `supabase/migrations/` 中的版本化迁移维护。`20260925000000_initial_schema.sql` 是基线，等同于引入迁移时生产库的完整结构。每次结构变更新增一个迁移文件，拉取请求需附验证结果及回滚方案。

首次启用迁移时，生产库已具备基线结构，只需登记而不执行：
...
1. 用 `supabase migration new <名称>` 创建迁移；优先新增兼容的字段、表、索引、函数、权限及 RLS，让旧版和新版应用都能运行。
2. `supabase db reset` 在本地从全部迁移重建，再载入测试数据并运行 `supabase test db`。
3. 应用依赖新结构时，先执行 `supabase db push` 并验证，再发布应用。
4. 检查正式站点及相关查询，后续版本再用新迁移移除旧代码不再使用的结构。

已提交且已应用的迁移不要修改，修正一律通过新迁移完成。破坏性变更前备份受影响数据，回滚方案应说明使用应用回滚、修复 SQL 或备份恢复。`supabase/archive/` 为历史增量 SQL，仅供追溯，不在新环境重复执行。
