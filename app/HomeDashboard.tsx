  ...
  return (
    <div className="container mx-auto px-6 md:px-8 max-w-7xl py-12 flex flex-col gap-6 animate-fade-in pt-24">
      ...
      <div className="flex flex-col gap-6 md:gap-8 mb-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-5">
          <div className="flex gap-1 bg-[#121212] p-1.5 rounded-xl border border-white/10 shadow-inner">
            {tabs.map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                  activeTab === tab
      ...
      <div className="w-full">
        ...
        {activeTab === "电影" && (
          <div key="movies" className="animate-fade-in grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
            <div className="col-span-1 md:col-span-3 bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col justify-between group">
              ...
              <div>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-4xl font-black text-white">{currentYearData?.watched_movies}</span>
                  <span className="text-sm text-neutral-400">/ {currentYearData?.total_movies} 部电影</span>
                </div>
                <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-red-500 rounded-full" style={{width: `${moviesPercent}%`}}></div>
                </div>
              </div>
            </div>
            <div className="col-span-1 md:col-span-1 bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col justify-between group">
            </div>
          </div>
        )}
