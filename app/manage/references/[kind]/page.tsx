...
export default async function ReferenceList({ params, searchParams }: { params: Promise<{ kind: string }>; searchParams: Promise<{ q?: string; page?: string; deleted?: string }> }) {
  ...
  return <section>
    <header className="mb-8 flex flex-wrap items-center justify-between gap-4"><div><p className="mb-2 text-sm text-neutral-400">关联资料</p><h1 className="admin-heading">{config.label}</h1><p className="mt-3 text-sm text-neutral-400">{count ?? 0} 项资料 · 集中维护，关联作品同步更新</p></div>{kind === "genres" || kind === "regions" ? <ReferenceQuickEdit kind={kind} /> : <Link className="admin-button admin-primary" href={`/manage/references/${kind}/new`}>新增{config.label}</Link>}</header>
