...
export default async function ReferenceList({ params, searchParams }: { params: Promise<{ kind: string }>; searchParams: Promise<{ q?: string; page?: string; deleted?: string }> }) {
  ...
  // 有别名的资料同时按名称和别名搜索。
  const { data, count, error } = await (config.alternate ? query.or(nameOrAliasFilter(q)) : query.ilike("name", `%${escapeLikePattern(q)}%`)).order("name").order("id").range((page - 1) * 25, page * 25 - 1);
  ...
  return <section>
    ...
    <form className="mb-6 flex items-end gap-3"><label className="flex-1">{config.alternate ? "搜索名称或别名" : "搜索名称"}<input name="q" defaultValue={q} maxLength={200} placeholder={config.alternate ? `搜索${config.label}名称或别名` : `搜索${config.label}`} /></label><button>搜索</button></form>
