...
export default async function NewMediaPage({ searchParams }: { searchParams: Promise<{ type?: string; parent?: string }> }) {
  ...
  const hasType = Boolean(params.type && Object.hasOwn(mediaTypes, params.type));
  const type = hasType ? params.type as ManagedMediaType : "movie";
  ...
  return <section><h1 className="admin-heading mb-8">新增{hasType ? mediaTypes[type] : "媒体"}</h1><MediaForm initialType={type} lockType={hasType} parent={parent} nextNumber={nextNumber} /></section>;
}
